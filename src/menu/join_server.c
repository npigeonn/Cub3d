...
/*   Updated: 2024/10/27 15:35:55 by ybeaucou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_join_server_menu_button(t_game *game, int mouse_x, int mouse_y)
{
	...
	const int btn_y_start = game->screen_height * 0.40 + (btn_height + spacing);
...
void	draw_join_server_menu(t_game *game)
{
	...
	info = init_draw_info(btn_height * 0.9, "Join Server", game->screen_width >> 1, y - 200);
	info.color = 0xFFFFF;
	draw_text(game, info);
	...
	const int total_btn_width = 2 * btn_half_width + game->screen_width * 0.02;
	const int btn_x_start = (game->screen_width - total_btn_width) / 2;
	const int btn_y_start = y + (btn_height + spacing);
