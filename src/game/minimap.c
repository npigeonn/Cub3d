...
/*   Updated: 2024/10/15 09:35:17 by npigeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mini_map(t_game *game)
{
	...
	while (y_minmap <= game->screen_height)
	{
		...
		while (x_minmap <= game->screen_width)
		{
			...
			pixel_put(game, x_minmap, y_minmap, grey_with_alpha);
...
void	mini_map(t_game *game)
{
	...
	while (y_minmap <= game->screen_height)
	{
		...
		while (x_minmap <= game->screen_width)
		{
			background_color = get_pixel_color_from_image(game, x_minmap, y_minmap);
			grey_with_alpha = blend_colors(background_color, grey, alpha);
			pixel_put(game, x_minmap, y_minmap, grey_with_alpha);
...
void	put_pixel_minimap(t_game *game, int x, int y, int size)
{
	...
	if (game->map[game->player->floor][y][x] == '1')
		...
		color = ((90 + 10 * ('z' - game->map[game->player->floor][y][x])) << 16) | (0 << 8) | 0;
	else
		color = 0;
	while (++i <= size)
	{
